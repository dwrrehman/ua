...
static void print_counts(void) {
	...
	printf("pm counts:\n");
		...
		printf("%13s: %-8lld\t\t", pm_spelling[i], pm_counts[i]);
        }
        puts("\n[done]");
}
...
static nat execute_graph_starting_at(
	...
	for (nat e = 0; e < execution_limit; e++) {
		...
		//1202608064.225218    prune the OS digit sequence:      starting with      3  2  1  6  2     

		if (e == 0 and op == three and os_sequence_index == 0) os_sequence_index = 1;
		else if (e == 1 and op == two and os_sequence_index == 1) os_sequence_index = 2;
		else if (e == 2 and op == one and os_sequence_index == 2) os_sequence_index = 3;
		else if (e == 3 and op == six and os_sequence_index == 3) os_sequence_index = 4;
		else if (e == 4 and op == two and os_sequence_index == 4) { return pm_os; }  // printf("pruned zv via pm_os!\n"); getchar(); 
		else os_sequence_index = 0;
		...
		if (op == one) {
			...
			if (pointer > xw and pointer < N) { 
				if (had_recently_expanded_pointer) { return pm_mri; }  // printf("[e = %llu]: pruned via mri\n", e); getchar(); 
				xw = pointer; 
			...
			if (comparator > max_sn) {
				if (had_recently_expanded_comparator) { return pm_mrsn; }  // printf("[e = %llu]: pruned via mrsn\n", e); getchar(); 
				max_sn = comparator;
...
static byte execute_graph(
	...
	for (byte o = 0; o < operation_count; o++) {
		...
		//puts("finished executing! pruned via "); puts(pm_spelling[pm]); puts("");
		//getchar();
...
static nat execute_graph_starting_at(byte origin, byte* graph, nat* array) {
	...
	for (; e < execution_limit; e++) {
		...
		if (op == one) {
			...
			if (BDL_counter >= max_bdl_er_repetions) return pm_b dl; 
...
printf("-------------------------------------execute_graph_starting_at(%hhu) {------------------------------------------\n", origin);
	...
	for (; e < stage1_execution_limit; e++) {
		...
		if (op == one) {
			...
			if (BDL_counter >= max_bdl_er_repetions)  return pm_bd l;
...
// #define max_consecutive_OSI_COUNT 40
	...
	byte	H0_counter = 0,  H1_counter = 0, 
		...
		OSI0_counter = 0, OSI1_counter = 0,
		OSI2_counter = 0, OSI3_counter = 0,
		OSI4_counter = 0, OSI5_counter = 0;
	...
	for (nat e = 0; e < execution_limit; e++) {
		...
		if (op == one) {
			...
			if (pointer == 1 or pointer == 2) { 
				if (last_mcal_op == three) {
					OSI0_counter++;
					if (OSI0_counter >= max_consecutive_OSI_COUNT) return pm_osi;
				} else OSI0_counter = 0;
			}
			...
			if (pointer == 3 or pointer == 4) { 
				if (last_mcal_op == three) {
					OSI1_counter++;
					if (OSI1_counter >= max_consecutive_OSI_COUNT) return pm_osi;
				} else OSI1_counter = 0;
			}


			if (pointer == 5 or pointer == 6) { 
				if (last_mcal_op == three) {
					OSI2_counter++;
					if (OSI2_counter >= max_consecutive_OSI_COUNT) return pm_osi;
				} else OSI2_counter = 0;
			}

			if (pointer == 7 or pointer == 8) { 
				if (last_mcal_op == three) {
					OSI3_counter++;
					if (OSI3_counter >= max_consecutive_OSI_COUNT) return pm_osi;
				} else OSI3_counter = 0;
			}


			if (pointer == 9 or pointer == 10) { 
				if (last_mcal_op == three) {
					OSI4_counter++;
					if (OSI4_counter >= max_consecutive_OSI_COUNT) return pm_osi;
				} else OSI4_counter = 0;
			}


			if (pointer == 11 or pointer == 12) { 
				if (last_mcal_op == three) {
					OSI5_counter++;
					if (OSI5_counter >= max_consecutive_OSI_COUNT) return pm_osi;
				} else OSI5_counter = 0;
			}
			...
			if (pointer + 3 == BDL_ier_at) {
				BDL4_counter++; 
				if (BDL4_counter >= max_consecutive_bld_walk_count_small) return pm_bdl; 
			} else BDL4_counter = 0;

			if (pointer + 2 == BDL_ier_at) {
				BDL5_counter++; 
				if (BDL5_counter >= max_consecutive_bld_walk_count_small) return pm_bdl; 
			} else BDL5_counter = 0;

			if (pointer + 1 == BDL_ier_at) {
				BDL6_counter++; 
				if (BDL6_counter >= max_consecutive_bld_walk_count_small) return pm_bdl; 
			} else BDL6_counter = 0;
			...
			if (last_mcal_op == one) {
				H0_counter++;
				if (H0_counter >= max_consecutive_h0_bouts) { 
					debug_pm(op, origin, e, history, pm_h0);
					fflush(stdout);
					return pm_h0;  
				} 
			}
...
#define max_consecutive_bld_walk_count 150
#define max_consecutive_bld_walk_count_medium 80
#define max_consecutive_bld_walk_count_small 30
#define max_consecutive_bld_walk_count_extra_small 8
...
pm_pair: 202503053.230813:


	/// PAIR_SEQUENCE #1   :    3 1 3 5      --->    { pair_count++; mcal_sequence_index = 255; } 
                                    

	/// PAIR_SEQUENCE #2   :    3 1 3 1 1    --->    pair_count++;
                                    ^
