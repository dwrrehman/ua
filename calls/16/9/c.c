...
// 2s: DOLs:    { 1, 2 }     { 1, 3 }     { 1, 5 }    { 2, 6 }    { 2, 5 }     { 1, 6 }    { 3, 5 }   { 3, 6 } 

static const u32 DOL_array[] = {   0x10, 0x20, 0x30, 0x41, 0x31, 0x40, 0x32, 0x42  };
	...
	for (nat i = 0; i < ltrdo_count; i += 8) {
		...
		prune: if (	not gi(g0, g1, x1) and 
			...
			/*if (pointer > 10) { 
				...
				printf("g0 = %016llx, g1 = %016llx\n", g0, g1);
				puts("");
				printf("VIA LTRDO: %llu, pointer = %hhu\n", i / 8, pointer); 
				getchar();
			}*/
			goto bad; 
	}
	...
	for (byte i = 0; i < operation_count; i++) {
		for (byte j = 0; j < 4 * 15; j += 4) {
			...
			if (	(graph[i] & 0xf) == A and
				...
				//for (nat _ = 0; _ < 15; _++) printf("%llu:%llu ", _, histogram[_]);
				//puts("");
				goto bad;
			}
		}
...
int main(void) {
	...
	const nat DOL_count = sizeof(DOL_array) / sizeof(*DOL_array);

for (nat dol_index = 0; dol_index < DOL_count; dol_index++) {
...
init:	pointer = 0;
	...
	// we might need to do job GA pruning here, 
	// in order to only be left with the jobs are 
	// guaranteed to take a decent amount of time 
	// and not be pruned instantly via GA. 
	...
	while (1) {
		...
		printf("\033[H\033[2J");
		printf("----------------- DOL 0x%x : %llu / %llu : jobs %llu / %llu -------------------\n", 
			DOL, dol_index, DOL_count, amount_remaining, total_job_count
		);
...
AT INIT, DOING GA:
	 ...
	 d1 o6  o5  o3 o2 o1
----------------------------------
g0 =  	001 14 000 000 00 00
        ^ C  A   7   4  2  0
	^

VIA LTRDO: 0, pointer = 14
