...
static byte graph_analysis(byte* graph, nat* counts) {
...
static void process_job(
	byte* job_begin, byte* job_end, 
	nat* array, 
	void* raw_graph, void* raw_end, 
	char* filename, nat* counts
) {
		...
		memcpy(graph, job_begin, graph_count);
		memcpy(end, job_end, graph_count);
		goto init;
	...
	init:  	pointer = lsepa;
		...
		byte at = graph_analysis(graph, counts);
		if (at) goto bad;
		byte origin = 0;
...
static void* worker_thread(void* raw_argument) {
	...
	for (nat job_index = 0; job_index < count; job_index++) {
		...
		process_job(jobs[job_index].begin, jobs[job_index].end, array, raw_graph, raw_end, filename, counts);
	}
