...
int main(int argc, const char * argv[]) {
    ...
    size_t lengths[s_count][s_count];
    memset(lengths, 0, sizeof(size_t) * s_count * s_count);    
    
    for (size_t s = 0; s < s_count; s++) {
        for (size_t w = 0; w < s_count; w++) {
    ...
    std::cout << "the longest CA was: (" << S << ", " << W << ") = " << biggest << "\n";
