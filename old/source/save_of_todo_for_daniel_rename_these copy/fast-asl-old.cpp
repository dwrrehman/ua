...
void print_vsbn(nats ns, nat m, nat n, nat l, nat j, nat nc) {        // ns is the indicies of neighbors, and has length = DE or SO or etc...
    for (nat i = 0; i < l; i++) {        
        if (i == j && !not_in(ns, nc, i)) printf("[Q]");
        else if (i == j) printf("[O]");            
        else if (!not_in(ns, nc, i)) printf("[X]");            
        else printf("[ ]");        
...
static void test_vsbn_code(nat l, nat m, nat n, nat nc, nat *ns) {
    for (nat j = 0; j < l; j++) {    
        ns[0] = j;
        nat y = 1;
        for (int k = 1; k < l; k *= m) 
            ns[y++] = j + k * ((j / k + 1) % m - j / k % m);        
        print_vsbn(ns, m, n, l, j, nc);
        printf("\n\n");
    }
}
