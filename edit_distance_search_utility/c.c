...
init:  	pointer = 0;
	...
	//printf("doing graph_analysis...\n");
	at = graph_analysis(graph, combination, counts);
	if (at) goto bad;


	//printf("doing execute_graph...\n");
	byte origin = 0;
	at = execute_graph(graph, array, &origin, counts);
	if (at) { at = 0; do at++; while (noneditable(at, combination)); }

	if (debug) {
		printf("\033[%dm%s:  at = %hhu, z = ", at ? 31 : 32, at ? "PRUNED" : " FOUND", at); 
		print_graph_raw(graph); 
...
bad:	// printf("doing a zskip at = %hhu\n", at); 
	// getchar(); 
	(*bad_count)++;
