...
static void* worker_thread(void* raw_argument) {
	...
	init:  	pointer = lsepa;
		...
		for (byte index = operation_count; index--;) {
			...
			if (index == g and graph[4 * index] != six) {
				at = graph_count;
		...
		for (byte index = 0; index < operation_count; index++) {
			byte connections = 0;
			for (byte offset = 1; offset < 4; offset++) {
				const byte x = graph[4 * index + offset];
				if (x != index) connections |= (1 << x);
			}
			for (byte other = 0; other < operation_count; other++) {
				if (other == index) continue;
				for (byte offset = 1; offset < 4; offset++) 
					if (graph[4 * other + offset] == index) connections |= (1 << other);
			}
			byte connection_count = 0;
			for (byte i = 0; i < operation_count; i++) {
				connection_count += (connections >> i) & 1;
			}
			if (connection_count != 6) {
				at = lsepa;
				counts[pm_ga_4o]++;
				//puts(pm_spelling[pm_ga_4o]);
				goto bad;
			} 
		}
